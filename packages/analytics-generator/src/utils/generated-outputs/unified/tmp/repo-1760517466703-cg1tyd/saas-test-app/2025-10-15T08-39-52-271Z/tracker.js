(function(root, factory) {
  ...
  class AnalyticsTracker {
    constructor() {
      ...
      this.config = {
        appKey: 'demo-test-apps-2025-10-15-ptl7s2ju1qd',
    ...
    initAutoTracking() {
      console.log('[demo-test-apps-2025-10-15-ptl7s2ju1qd] 🤖 AI-Enhanced Analytics initialized');
      console.log('[demo-test-apps-2025-10-15-ptl7s2ju1qd] 📊 Tracking 0 discovered components');
      console.log('[demo-test-apps-2025-10-15-ptl7s2ju1qd] 🔑 User ID:', this.userId);
      console.log('[demo-test-apps-2025-10-15-ptl7s2ju1qd] 📍 Session ID:', this.sessionId);
    ...
    extractContext(element, componentInfo) {
      ...
      for (const fieldConfig of componentInfo.context_collection.fields) {
        try {
          ...
          console.debug(`[demo-test-apps-2025-10-15-ptl7s2ju1qd] Context extraction failed for ${fieldConfig.field_name}`, error);
        }
      }
    ...
    flush() {
      ...
      .then(response => {
        if (!response.ok) {
          console.error('[demo-test-apps-2025-10-15-ptl7s2ju1qd] Analytics flush failed:', response.status);
          // Re-add to queue and persist on failure
        this.eventQueue.unshift(...batch);
          this.saveQueueToStorage();
        } else {
          console.debug('[demo-test-apps-2025-10-15-ptl7s2ju1qd] Flushed', batch.length, 'events');
        }
      })
      .catch(err => {
        console.error('[demo-test-apps-2025-10-15-ptl7s2ju1qd] Analytics flush error:', err);
        // Re-add to queue and persist on failure
        this.eventQueue.unshift(...batch);
        this.saveQueueToStorage();
      });
  ...
  if (typeof window !== 'undefined' && !window.analytics) {
    ...
    console.log('[demo-test-apps-2025-10-15-ptl7s2ju1qd] ✅ AI-Enhanced Analytics tracker with new event schema initialized');
  }
