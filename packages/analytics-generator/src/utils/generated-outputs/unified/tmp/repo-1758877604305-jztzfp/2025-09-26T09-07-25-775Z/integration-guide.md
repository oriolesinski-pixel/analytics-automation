# AI-Enhanced Analytics Integration Guide for test-app-rich-demo-2025-09-26-8521tkret0d
...
- **Button** (button): Primary action buttons (e.g. Continue Shopping, Log In, Create Account)
- **Link** (link): Secondary links (e.g. Sign in to existing account, Create new account)
- **QuantitySelector** (selector): Quantity control for cart items
- **RemoveFromCart** (button): Remove item from cart
- **EmailInput** (form_input): Email input field for login/registration
- **PasswordInput** (form_input): Password input field for login/registration
- **LoginForm** (form): Login form submission
- **RegisterForm** (form): Registration form submission
- **NameInput** (form_input): Name input field for registration
- **ConfirmPasswordInput** (form_input): Confirm password input field for registration
- **AddToCartButton** (button): Add product to cart
- **WishlistButton** (button): Add/remove product from wishlist
- **QuantityInput** (form_input): Quantity input for cart items
- **ClearCartButton** (button): Clear all items from cart
- **ProceedToCheckoutButton** (button): Proceed to checkout from cart
- **ShippingInfoForm** (form): Shipping information form submission
- **PaymentForm** (form): Payment form submission
- **PreviousSlideButton** (button): Navigate to previous slide in carousel
- **NextSlideButton** (button): Navigate to next slide in carousel
- **SlideIndicator** (button): Navigate to a specific slide in carousel
...
**Generated:** 2025-09-26T09:07:25.774Z
